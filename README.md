...
- **Persistencia con localStorage** — la colección se guarda entre sesiones
...
├── LOCALSTORAGE      → guardarEnStorage(), cargarDesdeStorage()
...
- **Sin innerHTML peligroso**: todo el DOM con `createElement` + `textContent`
- **Validación dual**: en tiempo real (evento `input`) + al enviar
- **Sanitización antes de almacenar**: datos limpios antes de entrar al arreglo
...
Desarrollado con asistencia de **Claude (Anthropic)**. Ver `USO_IA.md` para prompts y mejoras aplicadas.

- Arquitectura del módulo IIFE y funciones reutilizables
- Expresiones regulares con soporte español
- Algoritmo de análisis de imagen con Canvas API
- Sanitización y prevención XSS
